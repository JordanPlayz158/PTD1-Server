...
import xyz.jordanplayz158.ptd.server.module.ptd2.orm.PTD2Extra
import xyz.jordanplayz158.ptd.server.module.ptd2.orm.PTD2OneV1
import xyz.jordanplayz158.ptd.server.module.ptd2.orm.PTD2Pokemon
import xyz.jordanplayz158.ptd.server.module.ptd2.orm.PTD2Save
...
class Obfuscation {
    companion object {
        ...
        fun decodePokeInfo(encoded: String, id: Int): HashMap<Int, HashMap<String, Any>> {
            var id = id
        ...
        fun decodePokeInfo(encoded: String, story: PTD2Save): HashMap<Int, HashMap<String, Any>> {
            return decodePokeInfo(encoded, getAvailableSaveId(story))
        }


        fun decodePokeInfo(encoded: String, profile: PTD3Save): HashMap<Int, HashMap<String, Any>> {
            return decodePokeInfo(encoded, getAvailableSaveId(profile))
        }
        ...
        fun encodePokemons(pokemons: SizedIterable<PTD2Pokemon>): Pair<String, String> {
            ...
            for (poke in pokemons) {
                ...
                val (num, numLength) = convertValue(poke.number)
                val (xp, xpLength, xpLengthLength) = convertValueVariableLength(poke.experience)
                val (lvl, lvlLength) = convertValue(poke.level)
                ...
                val (pos, posLength) = convertValue(poke.position)
        ...
        fun encode1v1(profiles: SizedIterable<PTD2OneV1>): String {
            ...
            for (profile in profiles) {
                // May be wrong?
                val whichProfile = convertIntToString(profile.number)
                val encodedMoney = convertIntToString(profile.money)
                val moneyLength = convertIntToString(encodedMoney.length)

                val encodedLevels = convertIntToString(profile.levelsUnlocked)
        ...
        fun encodeStory(stories: SizedIterable<PTD2Save>?): String {
            val encodedData = StringBuilder()
            var profileSize = 0
        ...
        fun encodeStoryProfile(profile: PTD2Save): ArrayList<Any> {
            val encodedData = ArrayList<Any>(5)
            val extra = StringBuilder()

            val currentMap = convertIntToString(profile.currentMap)
            ...
            val extra2 = encodeInventory(profile.extras()) // Extra info is compatible with items encoding method
            val extra3 = encodePokemons(profile.pokemon)
            val extra4 = encodeInventory(profile.items())

            val extra5 = convertIntToString(
                createCheckSum("${extra3.first}0"))
        ...
        fun getAvailableSaveId(story: PTD2Save): Int {
            return transaction { (story.pokemon.maxByOrNull { pokemon -> pokemon.swfId }?.swfId ?: 0) + 1 }
        }

        fun getAvailableSaveId(profile: PTD3Save): Int {
            return transaction { (profile.pokemons.maxByOrNull { pokemon -> pokemon.swfId }?.swfId ?: 0) + 1 }
        }
    }
}
